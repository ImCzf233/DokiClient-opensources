...
import just.monika.主播你有反编译我代码的时间还不如自己写一个端子.config.LocalConfig;
...
public class LocalConfigRect extends GuiPanel {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks, int alpha) {
        ...
        FontUtil.iconFont26.drawString(FontUtil.SAVE, updateX, updateY,
                ColorUtil.interpolateColor(textColor, DokiClient.INSTANCE.getClientColor(), (float) hoverUpdateAnimation.getOutput()));

        if (hoveringUpdate || !hoverUpdateAnimation.isDone()) {
            int updateColor = ColorUtil.interpolateColor(ColorUtil.applyOpacity(DokiClient.INSTANCE.getClientColor(), 0),
                    DokiClient.INSTANCE.getClientColor(), (float) hoverUpdateAnimation.getOutput());
        ...
        FontUtil.iconFont26.drawString(FontUtil.LOAD, loadX, loadY,
                ColorUtil.interpolateColor(textColor, DokiClient.INSTANCE.getAlternateClientColor(), (float) hoverLoadAnimation.getOutput()));

        if (hoveringLoad || !hoverLoadAnimation.isDone()) {
            int loadColor = ColorUtil.interpolateColor(ColorUtil.applyOpacity(DokiClient.INSTANCE.getAlternateClientColor(), 0),
                    DokiClient.INSTANCE.getAlternateClientColor(), (float) hoverLoadAnimation.getOutput());
    ...
    public void mouseClicked(int mouseX, int mouseY, int button) {
        ...
        if (button == 0) {
            if (hoveringDelete) {
                if (DokiClient.INSTANCE.getConfigManager().delete(config.getName())) {
            ...
            if (hoveringUpdate) {
                if (DokiClient.INSTANCE.getConfigManager().saveConfig(config.getName())) {
            ...
            if (hoveringLoad) {
                if (DokiClient.INSTANCE.getConfigManager().loadConfig(DokiClient.INSTANCE.getConfigManager().readConfigData(config.getFile().toPath()))) {
