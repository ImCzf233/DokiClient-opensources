...
import just.monika.主播你有反编译我代码的时间还不如自己写一个端子.config.OnlineConfig;
...
public class ConfigPanel extends GuiPanel {
    ...
    public void initGui() {
        ...
        DokiClient.INSTANCE.getConfigManager().collectConfigs();
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks, int alpha) {
        ...
        mc.getTextureManager().bindTexture(new ResourceLocation("DokiClient/configbackground.png"));
        RoundedUtil.drawRoundTextured(x + 25, y + 70, rectWidth - 50, 75, 12, alpha / 255f);

        int textColor = ColorUtil.applyOpacity(-1, alpha / 255f);
        FontUtil.tenacityBoldFont40.drawCenteredString("DokiClient Configs", x + rectWidth / 2f, y + 77, textColor);
        ...
        finalSave.clickAction = () -> {
            if (DokiClient.INSTANCE.getConfigManager().saveConfig(stringField.getText())) {
        ...
        reloadButton.clickAction = () -> {
            DokiClient.INSTANCE.getConfigManager().collectOnlineConfigs();
            DokiClient.INSTANCE.getConfigManager().collectConfigs();
        ...
        FontUtil.tenacityFont16.drawCenteredString("To upload an online config, DM the DokiClient Bot \"/config submit\"",
    ...
    public void openConfigFolder() {
        try {
            Desktop.getDesktop().open(DokiClient.INSTANCE.getConfigManager().file);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
