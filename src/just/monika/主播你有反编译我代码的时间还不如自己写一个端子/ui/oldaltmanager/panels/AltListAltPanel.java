...
import just.monika.主播你有反编译我代码的时间还不如自己写一个端子.ui.oldaltmanager.panels.components.impl.Button;
import java.awt.*;
...
public class AltListAltPanel implements AltPanel {
    ...
    ArrayList<just.monika.主播你有反编译我代码的时间还不如自己写一个端子.ui.oldaltmanager.panels.components.Component> altObjects = new ArrayList<>();
    ArrayList<just.monika.主播你有反编译我代码的时间还不如自己写一个端子.ui.oldaltmanager.panels.components.Component> buttons = new ArrayList() {{
    ...
    public void initGui() {
        ...
        buttons.forEach(just.monika.主播你有反编译我代码的时间还不如自己写一个端子.ui.oldaltmanager.panels.components.Component::initGui);
        clickedPass = false;
    }
...
//        Draw.drawRoundedRect(xAnimation + width - 5, -scroll.getScroll() / scroll.getMaxScroll() * (height - scrollBarHeight) + 40, 3, scrollBarHeight, 1.5f, rectColorInt);//new Color(255, 255, 255, 120).getRGB()
    ...
    public void drawAltHead(Alt alt, float x, float y, float size) {
        ...
        mc.getTextureManager().bindTexture(alt.username == null ? new ResourceLocation("DokiClient/steve.png") : AbstractClientPlayer.getLocationSkin(alt.username));
        if (alt.email == null) {
