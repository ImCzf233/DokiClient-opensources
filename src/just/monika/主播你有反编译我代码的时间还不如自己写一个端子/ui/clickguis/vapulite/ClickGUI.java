...
public class ClickGUI extends GuiScreen {
    ...
    public ClickGUI() {
        ...
        for(final Category moduleCategory : Category.values()) {
            ...
            DokiClient.INSTANCE.getModuleCollection().getModules().stream().filter(module -> module.getCategory() == moduleCategory).forEach(module -> panel.addButton(new Button(panel, module)));
            panels.add(panel);
            x = x + 110;
        }
    }
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        // DO NOT DELETE THIS!
        fontRendererObj.drawStringWithShadow("DokiClient Author: Monika", 5, this.height - 20, new Color(255, 255, 255).hashCode());
