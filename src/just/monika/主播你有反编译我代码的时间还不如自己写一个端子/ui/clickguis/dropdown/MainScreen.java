...
import static just.monika.主播你有反编译我代码的时间还不如自己写一个端子.utils.misc.HoveringUtil.*;
...
public class MainScreen implements Screen {
    ...
    public void initGui() {
        if (moduleRects == null) {
            ...
            for (Module module : DokiClient.INSTANCE.getModuleCollection().getModulesInCategory(category).stream().sorted(Comparator.comparing(Module::getName)).collect(Collectors.toList())) {
    ...
    public void drawScreen(int mouseX, int mouseY) {
        ...
        ClickGuiMod clickGUIMod = (ClickGuiMod) DokiClient.INSTANCE.getModuleCollection().get(ClickGuiMod.class);
