...
import just.monika.主播你有反编译我代码的时间还不如自己写一个端子.ui.GuiEvents;
        ...
        for (Setting setting : module.getSettingsList()) {
            ...
            if (setting instanceof ColorSetting) {
                ...
                if (!openAnimation.isDone() || openAnimation.getDirection().equals(Direction.FORWARDS)) {
                    ...
                    mc.getTextureManager().bindTexture(new ResourceLocation("DokiClient/colorpicker2.png"));
                    Gui.drawModalRectWithCustomSizedTexture(pickerX, pickerY, 0, 0, 4, 4, 4, 4);


                    // Hue bar
                    RenderUtil.resetColor();
                    mc.getTextureManager().bindTexture(new ResourceLocation("DokiClient/hue.png"));
