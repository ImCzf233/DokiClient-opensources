...
import just.monika.主播你有反编译我代码的时间还不如自己写一个端子.ui.altmanager.panels.components.impl.Button;
import just.monika.主播你有反编译我代码的时间还不如自己写一个端子.ui.altmanager.panels.components.impl.StringField;
...
public class KingAltsPanel implements AltPanel {
    private final KingGenApi kingGenApi = DokiClient.INSTANCE.kingGenApi;
    ...
    public void mouseClicked(int mouseX, int mouseY, int button, AltManagerUtils altManagerUtils) {
        ...
        if (doesHaveKingAltKey) {
            ...
            if (hoveringGen) {
                ...
                ((AltListAltPanel) DokiClient.INSTANCE.altPanels.getPanel(AltListAltPanel.class)).reInitAltList();
            }
        }
