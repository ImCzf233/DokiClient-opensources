...
import just.monika.主播你有反编译我代码的时间还不如自己写一个端子.module.impl.movement.Speed;
import just.monika.主播你有反编译我代码的时间还不如自己写一个端子.utils.player.MovementUtils;
import just.monika.主播你有反编译我代码的时间还不如自己写一个端子.utils.player.RotationUtils;
...
public final class TargetStrafe extends Module {
    ...
    public static boolean canStrafe() {
        if (!DokiClient.INSTANCE.isToggled(TargetStrafe.class) || !MovementUtils.isMoving() || (space.isEnabled() && !Keyboard.isKeyDown(Keyboard.KEY_SPACE))) {
            return false;
        }
        if (!(DokiClient.INSTANCE.isToggled(Speed.class) || DokiClient.INSTANCE.isToggled(Flight.class))) {
            return false;
        }
        return DokiClient.INSTANCE.isToggled(KillAura.class)
                && ((KillAura) (DokiClient.INSTANCE.getModuleCollection().get(KillAura.class))).isValid(KillAura.target)
                && DokiClient.INSTANCE.isToggled(TargetStrafe.class)
