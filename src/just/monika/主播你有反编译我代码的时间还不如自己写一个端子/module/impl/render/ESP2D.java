...
import just.monika.主播你有反编译我代码的时间还不如自己写一个端子.utils.render.*;
...
public class ESP2D extends Module {
    ...
    private final ModeSetting boxColorMode = new ModeSetting("Box Mode", "Sync", "Sync", "DokiClient", "Light Rainbow", "Rainbow", "Static", "Fade", "Double Color", "Analogous");
        ...
        if (boxEsp.isEnabled()) {
            switch (boxColorMode.getMode()) {
                case "Sync":
                    ...
                    break;
                case "DokiClient":
                    firstColor = ColorUtil.interpolateColorsBackAndForth(15, 0, DokiClient.INSTANCE.getClientColor(), DokiClient.INSTANCE.getAlternateClientColor(), HudMod.hueInterpolation.isEnabled());
                    secondColor = ColorUtil.interpolateColorsBackAndForth(15, 90, DokiClient.INSTANCE.getClientColor(), DokiClient.INSTANCE.getAlternateClientColor(), HudMod.hueInterpolation.isEnabled());
                    thirdColor = ColorUtil.interpolateColorsBackAndForth(15, 180, DokiClient.INSTANCE.getClientColor(), DokiClient.INSTANCE.getAlternateClientColor(), HudMod.hueInterpolation.isEnabled());
                    fourthColor = ColorUtil.interpolateColorsBackAndForth(15, 270, DokiClient.INSTANCE.getClientColor(), DokiClient.INSTANCE.getAlternateClientColor(), HudMod.hueInterpolation.isEnabled());
                    break;
                case "Light Rainbow":
