...
import just.monika.主播你有反编译我代码的时间还不如自己写一个端子.utils.misc.StringUtils;
...
public class ArraylistMod extends Module {
    ...
    public void getModules() {
        if (modules == null) {
            List<Class<? extends Module>> hiddenModules = DokiClient.INSTANCE.getModuleCollection().getHiddenModules();
            List<Module> moduleList = DokiClient.INSTANCE.getModuleCollection().getModules();
            moduleList.removeIf(module -> hiddenModules.stream().anyMatch(moduleClass -> moduleClass == module.getClass()));
            modules = moduleList;
        }
    }

    public Dragging arraylistDrag = DokiClient.INSTANCE.createDrag(this, "arraylist", 2, 3);
    ...
    private final EventListener<Render2DEvent> onRender2D = e -> {
        ...
        for (Module module : modules) {
            ...
            switch (HudMod.colorMode.getMode()) {//"DokiClient", "Light Rainbow", "Rainbow", "Static", "Fade", "Double Color", "Analogous"
                case "DokiClient":
                    textcolor = ColorUtil.interpolateColorsBackAndForth(15, count * 20, DokiClient.INSTANCE.getClientColor(), DokiClient.INSTANCE.getAlternateClientColor(), HudMod.hueInterpolation.isEnabled());
                    break;
                case "Light Rainbow":
