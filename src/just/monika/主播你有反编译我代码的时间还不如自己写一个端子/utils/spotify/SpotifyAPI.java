package just.monika.主播你有反编译我代码的时间还不如自己写一个端子.utils.spotify;
...
public class SpotifyAPI implements Utils {
    ...
    //.setLenient()
    public final static File CLIENT_ID_DIR = new File(DokiClient.DIRECTORY, "SpotifyID.json");
    ...
    public void startConnection() {
        if (!authenticated) {
            try {
                ...
                DokiClient.INSTANCE.getExecutorService().submit(() -> {
                    try {
                        ...
                        callbackServer.createContext("/", context -> {
                            ...
                            final String messageSuccess = context.getRequestURI().getQuery().contains("code")
                                    ? "Successfully authorized.\nYou can now close this window, have fun on DokiClient!"
